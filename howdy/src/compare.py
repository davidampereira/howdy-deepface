...
try:
    ...
    print(
        _(
            "Stored face models are not compatible with the selected recognition backend."
        )
    )
    print(
        _("Please re-enroll your face model with: sudo howdy clear && sudo howdy add")
    )
    exit(10)
...
while True:
    ...
    for result in results:
        ...
        if face_encoding.ndim != 1 or face_encoding.shape[0] != expected_embedding_dim:
            ...
            print(
                _(
                    "Please re-enroll your face model with: sudo howdy clear && sudo howdy add"
                )
            )
            exit(10)
        ...
        if 0 < match < video_certainty:
            ...
            if config.getboolean("rubberstamps", "enabled", fallback=False):
                ...
                rubberstamps.execute(
                    config, gtk_proc, {"video_capture": video_capture, "clahe": clahe}
                )
